...
pub fn writeMessage(file: std.fs.File, json_text: []const u8) !void {
...
pub fn readMessageAlloc(allocator: std.mem.Allocator, file: std.fs.File) ![]u8 {
    var content_length: ?usize = null;
    while (true) {
        const line = try readLineAlloc(allocator, file);
    ...
    while (read_index < body.len) {
        const amount = try file.read(body[read_index..]);
...
fn readLineAlloc(allocator: std.mem.Allocator, file: std.fs.File) ![]u8 {
    var line: std.ArrayListUnmanaged(u8) = .empty;
    errdefer line.deinit(allocator);

    while (true) {
        var buf: [1]u8 = undefined;
        const amount = try file.read(buf[0..]);
        if (amount == 0) {
